
# twitter
configatron.twitter.consumer_key    = 'EfyBrxvjIq4HptaLhkPPg'
configatron.twitter.consumer_secret = '<your consumer secret>'
configatron.twitter.oauth_callback  = { :oauth_callback => 'http://kurorekishi.yabasoft.biz/login' }

# PRTools
configatron.prtools.twitter.consumer_key    = '4upSvEDhihcbIB9VdXBIQ'
configatron.prtools.twitter.consumer_secret = '<your consumer secret>'
configatron.prtools.twitter.access_token        = '556482467-AjJwFCopjp8Y45ZV6KcwVoljLe0XT4Sioq6e9IHD'
configatron.prtools.twitter.access_token_secret = '<your accesstoken secret>'
